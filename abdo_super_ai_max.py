...
if not st.session_state["auth"]:
    ...
    pwd = st.text_input("أدخلي كلمة المرور الخاصة بك:", type="password")
...
# --- بعد تسجيل الدخول ---
st.markdown("<h1 class='title'>ABDO SUPER Ai</h1>", unsafe_allow_html=True)

# --- رسالة ترحيب تلقائية ---
def welcome_message():
    msg = "مرحبًا بك يا سيدتي ABDO! 👑\nABDO SUPER Ai جاهز لتنفيذ أوامرك."
    display_text = ""
    for char in msg:
        display_text += char
...
ROOT = "./ABDO_SUPER_AI"
for p in ["projects","exploits","vault","media","terminal","scrapers","videos"]:
...
def autopilot_execute(command, uploaded_file=None, speak=False):
    ...
    try:
        ...
        # تحليل المواقع
        elif "حلل موقع" in cmd or "سحب بيانات" in cmd:
            url = "https://example.com"
            r = requests.get(url)
            data = BeautifulSoup(r.text, "html.parser").get_text()[:1000]
            output = f"🌐 بيانات الموقع:\n{data}"
        # رفع ملفات صور/فيديو
        ...
        # أكواد
        elif "اكواد" in cmd or "code" in cmd:
            output = f"💻 تم توليد كود تلقائيًا حسب الطلب: {command}"
        else:
...
with col2:
    query = st.text_input("أصدري أمرك يا سيدة ABDO…")
with col3:
...
# --- تنفيذ الأمر عند الضغط على إرسال أو عند رفع ملف ---
